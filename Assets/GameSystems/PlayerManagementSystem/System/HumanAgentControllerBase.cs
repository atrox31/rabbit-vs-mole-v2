...
namespace PlayerManagementSystem
{
    ...
    /// <typeparam name="TypeOfHumanAvatar">The type of the player avatar associated with the human agent. Must inherit from <see cref="PlayerAvatarBase"/>.</typeparam>
    public abstract class HumanAgentControllerBase<TypeOfPlayer, TypeOfHumanAvatar> : AgentController<TypeOfPlayer> 
        where TypeOfPlayer : Enum 
        where TypeOfHumanAvatar : PlayerAvatarBase
    {
        ...
        private bool SetGamepadScheme()
        {
            if (_myGamepad == null)
            {
                if (!InputDeviceManager.TryToGetGamepadDevice(out Gamepad gamepadDevice))
                {
        ...
        private void OnDestroy()
        {
            ...
            if (_myGamepad != null)
            {
                InputDeviceManager.ReleaseGamepad(_myGamepad);
            }
        }
